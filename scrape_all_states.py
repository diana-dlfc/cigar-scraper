...
# Scrape cigar lounges por estado via Yelp → Supabase
#
# Run:
#   venv\Scripts\python scrape_all_states.py            # todos los estados
#   venv\Scripts\python scrape_all_states.py TX CA NY   # estados específicos
...
# Estados en orden alfabético por nombre
...
print(f"\n{'='*60}")
print(f"  Cigar Scraper — {len(TARGET_STATES)} estados")
...
for i, state_abbr in enumerate(TARGET_STATES, 1):
    ...
    for city in cities:
        ...
        try:
            results = search_city(city, state_abbr, fetch_details=False)
...
print(f"  Estados: {len(TARGET_STATES)}")
